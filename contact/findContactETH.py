...
def writeMshFile(filename, verts, centroid, edges, physical, elemFields, elemData, nodeFields, nodeData):
    ...
    with open(filename, 'w') as file:
        ...
        for i in range(0, len(nodeFields)):
            writeNodeData(file, verts, nodeData[i], nodeFields[i]) 
...
def main(files,tag):
    for file in files:
        ...
        idpair,unitvec,area,delta,radpair = findContacts(ids, rad, centroid)
        ...
        writeMshFile(outFile, ids, centroid, idpair, physical, elemFields, elemData, nodeFields, nodeData)
    return
